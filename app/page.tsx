...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (props) {
        setAllProperties(props.map((p: any) => ({ property_id: p.property_id, address: p.address, city: p.city })))
        setFullProperties(props)
  ...
  const heroKpis = (() => {
    if (fullProperties.length === 0) return null

    const isAllProperties = !selectedPropertyId || selectedPropertyId === ''
    const props = isAllProperties
      ? fullProperties
      : fullProperties.filter((p: any) => p.property_id === selectedPropertyId)

    if (props.length === 0) return null

    let totCashInvested = 0, totNetCashInvested = 0, totEquity = 0
    let totCashflow = 0, totMortgage = 0, totMV = 0
    ...
    for (const p of props) {
      ...
      const mortgage = (p.revaluation_estimate || 0) * (1 - (p.deposit_pct_phase2 || 0))
      const mortgageInterest = mortgage * (p.mortgage_rate_phase2 || 0)
      const opCosts = (p.management_phase2 || 0) + (p.provision_costs_phase2 || 0) +
        (p.provision_voids_phase2 || 0) + (p.bills_phase2 || 0)
      const cashflow = (p.annual_rent_phase2 || 0) - opCosts - mortgageInterest
      const eq = (p.market_value_est || 0) - mortgage

      totCashInvested += totalCash
      totNetCashInvested += netCash
      totEquity += eq
      totCashflow += cashflow
      totMortgage += mortgage
      totMV += p.market_value_est || 0
    }

    // Net P&L from transactions (filtered by property but NOT by date)
    let txnNetPnL: number
    if (isAllProperties) {
      txnNetPnL = data.reduce((sum: number, row: any) => sum + Number(row['Item amount inc VAT'] || 0), 0)
    } else {
      const selProp = allProperties.find(p => p.property_id === selectedPropertyId)
      if (selProp) {
        const addrKey = selProp.address.split(',')[0].trim().toLowerCase()
        txnNetPnL = data
          .filter((row: any) => String(row['Property address'] || '').toLowerCase().includes(addrKey))
          .reduce((sum: number, row: any) => sum + Number(row['Item amount inc VAT'] || 0), 0)
      } else {
        txnNetPnL = 0
      }
    }
  ...
  // ─── PropertySelector mode derived from active tab ────────────────────────
  const psMode: 'summary' | 'detail' = activeTab === 'Summary' ? 'summary' : 'detail'
  const showPropertySelector = activeTab === 'Summary' || activeTab === 'Property Detail'
  ...
  return (
    ...
    <div style={{ padding: '36px 48px', fontFamily: FONT, background: BG, minHeight: '100vh', color: TEXT }}>
      ...
      <div style={{
        ...
        marginBottom: 28, paddingBottom: 24, borderBottom: `1px solid ${BORDER}`,
      }}>
        <div>
      ...
      {/* ── HERO KPI STRIP (Summary tab only) ────────────────────────────── */}
      {activeTab === 'Summary' && heroKpis && (
        <div style={{ display: 'flex', gap: 20, marginBottom: 44 }}>
          {/* ROI */}
          <div style={{ flex: 1, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, padding: '24px 28px' }}>
            <div style={{ fontSize: 10, color: TEXT3, textTransform: 'uppercase', letterSpacing: '2px', marginBottom: 14, fontWeight: 600 }}>
              Cash-on-Cash ROI
            </div>
            <div style={{
              fontSize: 38, fontWeight: 700, letterSpacing: '-1px', lineHeight: 1,
              color: heroKpis.roi >= 10 ? GREEN : heroKpis.roi >= 0 ? AMBER : RED,
              fontVariantNumeric: 'tabular-nums',
            }}>
              {heroKpis.roi.toFixed(1)}%
            </div>
            <div style={{ fontSize: 11, color: TEXT3, marginTop: 12 }}>
              {heroKpis.isAllProperties ? `${heroKpis.propertyCount} properties weighted` : 'vs business case'}
            </div>
          </div>
          ...
          <div style={{ flex: 1, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, padding: '24px 28px' }}>
            ...
            <div style={{
              fontSize: 38, fontWeight: 700, letterSpacing: '-1px', lineHeight: 1,
              color: heroKpis.equityMultiple >= 1 ? GREEN : AMBER,
              fontVariantNumeric: 'tabular-nums',
            }}>
              {heroKpis.equityMultiple.toFixed(2)}x
            </div>
            <div style={{ fontSize: 11, color: TEXT3, marginTop: 12 }}>
              equity + cashflow vs total deployed
            </div>
          </div>
          ...
          <div style={{ flex: 1, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, padding: '24px 28px' }}>
            ...
            <div style={{
              fontSize: 38, fontWeight: 700, letterSpacing: '-1px', lineHeight: 1,
              color: heroKpis.ltv <= 75 ? GREEN : heroKpis.ltv <= 85 ? AMBER : RED,
              fontVariantNumeric: 'tabular-nums',
            }}>
              {heroKpis.ltv.toFixed(1)}%
            </div>
            <div style={{ fontSize: 11, color: TEXT3, marginTop: 12 }}>
              {heroKpis.isAllProperties ? 'portfolio LTV' : 'property LTV'}
            </div>
          </div>
          ...
          <div style={{ flex: 1, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, padding: '24px 28px' }}>
            ...
            <div style={{
              fontSize: 38, fontWeight: 700, letterSpacing: '-1px', lineHeight: 1,
              color: heroKpis.monthlyCashflow >= 0 ? GREEN : RED,
              fontVariantNumeric: 'tabular-nums',
            }}>
              £{fmt(heroKpis.monthlyCashflow)}
            </div>
            <div style={{ fontSize: 11, color: TEXT3, marginTop: 12 }}>
              {heroKpis.isAllProperties
                ? `£${fmt(heroKpis.totCashflow)} p.a. · ${heroKpis.propertyCount} properties`
                : `£${fmt(heroKpis.totCashflow)} p.a.`}
            </div>
          </div>
        </div>
      )}

      {/* ── PROPERTY ANALYSIS (Summary + Property Detail tabs) ───────────── */}
      <div style={{ display: showPropertySelector ? 'block' : 'none' }}>
        <PropertySelector
          mode={psMode}
      ...
      {/* ── PROPERTY DETAIL TAB — P&L sections ───────────────────────────── */}
